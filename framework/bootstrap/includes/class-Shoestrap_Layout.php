<?php
// 483 linijka - przy layoucie ustawionym na Fluid w menu można wybrać normal i dzięki temu do menu zostanie przypisana klasa container. Wcześniej niezależnie od ustawienia
// szerokości menu, do menu dodawana była klasa fluid

// 460 - dodałem funkcję, która w przypadku ustawienia szerokości strony na fluid oraz gdy na stronie umieścimy sidebar, to szerokość strony zostanie ustawiona na container
	...
	*/
	class Shoestrap_Layout {
		...
		function options( $sections ) {
			...
			$fields[] = array(
				'title'     => __( 'Główny styl layoutu strony', 'shoestrap' ),
				'desc'      => __( 'Domyślnie "Wide".', 'shoestrap' ),
				'id'        => 'site_style',
				'default'   => 'wide',
				'type'      => 'select',
				'options'   => array(
		...
		public static function container_class() {
			...
			$class    = $ss_settings['site_style'] != 'fluid' ? 'container' : 'fluid';
 			if ( shoestrap_display_primary_sidebar() || shoestrap_display_secondary_sidebar() ) {
 				$class    = $ss_settings['site_style'] != 'fluid' ? 'container' : 'container';
 			}
		...
		function navbar_container_class() {
			...
			if ( $toggle == 'full' ) {
				...
				$class = ( $site_style != 'fluid' ) ? 'container' : 'container';
			}
